...
try {
    ...
    logUninstall('Uninstall: Checked for shrinkr1_link table - exists: ' . ($urlTableExists ? 'YES' : 'NO'));
} catch (\Exception $e) {
...
try {
    ...
    foreach ($demoUrlIDs as $linkID) {
        try {
            ...
            if ($hash && strpos($hash, 'DEMO-') === 0) {
                ...
                logUninstall('SECURITY WARNING - URL ID ' . $linkID . ' has hash "' . ($hash ?? 'NULL') . '" which does not start with "DEMO-", skipping deletion');
            }
        } catch (\Exception $e) {
            logUninstall('Error verifying URL ID ' . $linkID . ': ' . $e->getMessage());
        }
    }
    ...
    try {
        ...
        if ($featuredLinkTableExists && !empty($demoUrlIDs)) {
            ...
            $sql = "DELETE fl FROM shrinkr1_featured_link fl 
                    INNER JOIN shrinkr1_link u ON fl.linkID = u.linkID 
                    WHERE fl.linkID IN ({$placeholders}) AND u.hash LIKE 'DEMO-%'";
            $statement = WCF::getDB()->prepareStatement($sql);
    ...
    try {
        ...
        if ($customButtonTableExists && !empty($demoUrlIDs)) {
            ...
            // Additional safety: Join with url table to ensure hash starts with DEMO-
            $placeholders = str_repeat('?,', count($demoUrlIDs) - 1) . '?';
            $sql = "DELETE cb FROM shrinkr1_custom_button cb 
                    INNER JOIN shrinkr1_link u ON cb.linkID = u.linkID 
                    WHERE cb.linkID IN ({$placeholders}) AND u.hash LIKE 'DEMO-%'";
            $statement = WCF::getDB()->prepareStatement($sql);
            $statement->execute($demoUrlIDs);
    ...
    try {
        ...
        if ($specialTableExists && !empty($demoUrlIDs)) {
            ...
            // Additional safety: Join with url table to ensure hash starts with DEMO-
            $placeholders = str_repeat('?,', count($demoUrlIDs) - 1) . '?';
            $sql = "DELETE s FROM shrinkr1_special s 
                    INNER JOIN shrinkr1_link u ON s.linkID = u.linkID 
                    WHERE s.linkID IN ({$placeholders}) AND u.hash LIKE 'DEMO-%'";
            $statement = WCF::getDB()->prepareStatement($sql);
            $statement->execute($demoUrlIDs);
    ...
    try {
        if (!empty($demoUrlIDs)) {
            ...
            logUninstall('Deleted ' . $deletedUrls . ' demo URLs (by verified linkID with DEMO- hash check - SAME METHOD AS OPTION LISTENER)');
            ...
            try {
                $sql = "DELETE FROM shrinkr1_link WHERE hash LIKE 'DEMO-%'";
                $statement = WCF::getDB()->prepareStatement($sql);
                $statement->execute();
