 ...
 * 
 * 1. Sets shrinkr_normal_page_mode to true (1) for full page mode
...
// ============================================================================
// 1. Set shrinkr_normal_page_mode to 1 (true) for full page mode
// ============================================================================
...
$tableExists = false;
try {
    $sql = "SHOW TABLES LIKE 'shrinkr1_description'";
    $statement = WCF::getDB()->prepareStatement($sql);
    $statement->execute();
...
if ($tableExists) {
    try {
        $sql = "SELECT COUNT(*) FROM shrinkr1_description";
        $statement = WCF::getDB()->prepareStatement($sql);
        $statement->execute();
        ...
        if ($count == 0) {
            ...
            $sql = "INSERT INTO shrinkr1_description (title, descriptionText, isActive) VALUES (?, ?, 1)";
            $statement = WCF::getDB()->prepareStatement($sql);
...
try {
    $sql = "SHOW TABLES LIKE 'shrinkr1_theme'";
    $statement = WCF::getDB()->prepareStatement($sql);
    $statement->execute();
...
if ($themeTableExists) {
    try {
        $sql = "SELECT COUNT(*) FROM shrinkr1_theme";
        $statement = WCF::getDB()->prepareStatement($sql);
        $statement->execute();
        $count = $statement->fetchSingleColumn();
        ...
        if ($count == 0) {
            ...
            $sql = "INSERT INTO shrinkr1_theme
...
try {
    $sql = "SHOW TABLES LIKE 'shrinkr1_link'";
    $statement = WCF::getDB()->prepareStatement($sql);
    $statement->execute();
...
try {
    $demoDataOption = \wcf\data\option\Option::getOptionByName('shrinkr_install_demo_data');
...
$fromEvent = isset($GLOBALS['shrinkr_demo_data_from_event']) && $GLOBALS['shrinkr_demo_data_from_event'];
    ...
    try {
        ...
        if ($discountTableExists) {
            try {
                ...
                $sql = "SELECT COUNT(*) FROM shrinkr1_discount WHERE hosts = ?";
                $statement = WCF::getDB()->prepareStatement($sql);
                ...
                if ($count == 0) {
                    ...
                    $sql = "INSERT INTO shrinkr1_discount
        ...
        while ($row = $statement->fetchArray()) {
            $existingDemoUrls[$row['hash']] = $row['linkID'];
        }
        ...
        if ($isDemoColumnExists) {
            $sql = "SELECT linkID, hash FROM shrinkr1_link WHERE isDemo = 1";
            $statement = WCF::getDB()->prepareStatement($sql);
            $statement->execute();
            while ($row = $statement->fetchArray()) {
                $insertedUrlIDs[$row['hash']] = $row['linkID'];
                logPostInstall('Post-install: Found existing demo URL ' . $row['hash'] . ' with ID ' . $row['linkID'] . ' (by isDemo flag)');
            }
        }
        ...
        while ($row = $statement->fetchArray()) {
            if (!isset($insertedUrlIDs[$row['hash']])) {
                $insertedUrlIDs[$row['hash']] = $row['linkID'];
                logPostInstall('Post-install: Found existing URL ' . $row['hash'] . ' with ID ' . $row['linkID'] . ' (by hash)');
            }
            
            // Mark existing demo URLs with isDemo flag if column exists
            if ($isDemoColumnExists) {
                try {
                    $updateSql = "UPDATE shrinkr1_link SET isDemo = 1 WHERE linkID = ?";
                    $updateStatement = WCF::getDB()->prepareStatement($updateSql);
                    $updateStatement->execute([$row['linkID']]);
                    logPostInstall('Post-install: Marked existing URL ' . $row['hash'] . ' (ID: ' . $row['linkID'] . ') as demo (isDemo = 1)');
                } catch (\Exception $e) {
        ...
        // 5.1 Create comprehensive test discounts (only if discount table exists)
            ...
            $sql = "SELECT COUNT(*) FROM shrinkr1_discount WHERE discountValue LIKE '%Special%' OR discountValue LIKE '%Deals%'";
            $statement = WCF::getDB()->prepareStatement($sql);
            $statement->execute();
            ...
            foreach ($testDiscounts as $discount) {
                ...
                $insertedDiscountIDs[] = WCF::getDB()->getInsertID('shrinkr1_discount', 'discountID');
            }
            }
            }
            ...
            // Use direct SQL to create URLs (ShrinkrLinkAction might not work during installation)
            foreach ($testUrls as $url) {
                try {
                    // Check if URL with this hash already exists
                    $sql = "SELECT linkID FROM shrinkr1_link WHERE hash = ?";
                    $statement = WCF::getDB()->prepareStatement($sql);
                    ...
                    $sql = "SHOW COLUMNS FROM shrinkr1_link";
                    $statement = WCF::getDB()->prepareStatement($sql);
                    $statement->execute();
                    $columns = [];
                    while ($row = $statement->fetchArray()) {
                    ...
                    if (in_array('linkTitle', $columns)) {
                        $insertColumns[] = 'linkTitle';
                        $insertValues[] = '?';
                    ...
                    $sql = "INSERT INTO shrinkr1_link (" . implode(', ', $insertColumns) . ") VALUES (" . implode(', ', $insertValues) . ")";
                    logPostInstall('Post-install: SQL: ' . $sql);
                    logPostInstall('Post-install: Params: ' . print_r($params, true));
                    $statement = WCF::getDB()->prepareStatement($sql);
                    $statement->execute($params);
                    
                    $newUrlID = WCF::getDB()->getInsertID('shrinkr1_link', 'linkID');
                    if ($newUrlID) {
            ...
            while ($row = $statement->fetchArray()) {
                if (!isset($insertedUrlIDs[$row['hash']])) {
                    $insertedUrlIDs[$row['hash']] = $row['linkID'];
                    logPostInstall('Post-install: Found missing URL ' . $row['hash'] . ' with ID ' . $row['linkID']);
                }
                ...
                if ($isDemoColumnExists) {
                    try {
                        ...
                        $updateStatement = WCF::getDB()->prepareStatement($updateSql);
                        $updateStatement->execute([$row['linkID']]);
                        logPostInstall('Post-install: Marked URL ' . $row['hash'] . ' (ID: ' . $row['linkID'] . ') as demo (isDemo = 1)');
                    } catch (\Exception $e) {
        ...
        if (!empty($insertedUrlIDs)) {
            ...
            if ($specialTableExists) {
                ...
                $sql = "SELECT COUNT(*) FROM shrinkr1_special WHERE linkID IN (" . implode(',', array_values($insertedUrlIDs)) . ")";
                $statement = WCF::getDB()->prepareStatement($sql);
                $statement->execute();
                ...
                $sql = "INSERT INTO shrinkr1_special
                        (linkID, theme, title, discount, discountID, codes, primaryColor, secondaryColor, primaryTextColor, secondaryTextColor, additionalText, startTime, endTime, isActive)
                ...
                $statement = WCF::getDB()->prepareStatement($sql);
                    ...
                    foreach ($testSpecials as $special) {
                        try {
                            ...
                            logPostInstall('Post-install: Created special for URL ID ' . $special['linkID']);
                        } catch (\Exception $e) {
                            logPostInstall('Post-install: Failed to create special for URL ID ' . $special['linkID'] . ': ' . $e->getMessage());
                            logPostInstall('Post-install: Stack trace: ' . $e->getTraceAsString());
                        }
                    }
                } else {
            ...
            if ($featuredLinkTableExists) {
                ...
                $sql = "INSERT INTO shrinkr1_featured_link
                        (linkID, url, title, sortOrder)
                        VALUES (?, ?, ?, ?)";
                $statement = WCF::getDB()->prepareStatement($sql);
                        ...
                        foreach ($testFeaturedLinks as $link) {
                            ...
                            if (!isset($link['linkID']) || !$link['linkID']) {
                                $skippedCount++;
                            ...
                            try {
                                $checkSql = "SELECT COUNT(*) FROM shrinkr1_featured_link WHERE linkID = ? AND url = ? AND title = ?";
                                $checkStatement = WCF::getDB()->prepareStatement($checkSql);
                                $checkStatement->execute([$link['linkID'], $link['url'], $link['discountValue']]);
                                $exists = $checkStatement->fetchSingleColumn();
                                
                                if ($exists > 0) {
                                    $skippedCount++;
                                    logPostInstall('Post-install: ⚠ Featured link already exists, skipping: ' . $link['discountValue'] . ' for URL ID ' . $link['linkID']);
                                    continue;
                                }
                            } catch (\Exception $e) {
                            ...
                            try {
                                ...
                                $createdCount++;
                                logPostInstall('Post-install: ✓ Created featured link "' . $link['discountValue'] . '" for URL ID ' . $link['linkID']);
                            } catch (\Exception $e) {
                                $skippedCount++;
                                logPostInstall('Post-install: ✗ Failed to create featured link for URL ID ' . $link['linkID'] . ': ' . $e->getMessage());
                                logPostInstall('Post-install: Stack trace: ' . $e->getTraceAsString());
                            }
                        }
            ...
            if ($customButtonTableExists && !empty($insertedUrlIDs)) {
                ...
                if (!empty($insertedUrlIDs)) {
                    ...
                    if (!empty($insertedUrlIDs)) {
                        ...
                        $sql = "INSERT INTO shrinkr1_custom_button
                                (linkID, targetUrl, title, sortOrder)
                                VALUES (?, ?, ?, ?)";
                        $statement = WCF::getDB()->prepareStatement($sql);
                        ...
                        foreach ($testCustomButtons as $button) {
                            try {
                                ...
                                if (!isset($button['linkID']) || !$button['linkID']) {
                                    $skippedCount++;
                                ...
                                try {
                                    $checkSql = "SELECT COUNT(*) FROM shrinkr1_custom_button WHERE linkID = ? AND targetUrl = ? AND title = ?";
                                    $checkStatement = WCF::getDB()->prepareStatement($checkSql);
                                    $checkStatement->execute([$button['linkID'], $button['targetUrl'], $button['title']]);
                                    $exists = $checkStatement->fetchSingleColumn();
                                    
                                    if ($exists > 0) {
                                        $skippedCount++;
                                        logPostInstall('Post-install: ⚠ Custom button already exists, skipping: ' . $button['title'] . ' for URL ID ' . $button['linkID']);
                                        continue;
                                    }
                                } catch (\Exception $e) {
                                    // If check fails, try to create anyway
                                ...
                                $createdCount++;
                                logPostInstall('Post-install: ✓ Created custom button "' . $button['title'] . '" for URL ID ' . $button['linkID']);
                            } catch (\Exception $e) {
                                $skippedCount++;
                                logPostInstall('Post-install: ✗ Failed to create custom button for URL ID ' . ($button['linkID'] ?? 'UNKNOWN') . ': ' . $e->getMessage());
                                logPostInstall('Post-install: Stack trace: ' . $e->getTraceAsString());
                            }
                        }
