...
print_section "WoltLab Snapshot Erstellen" "Hauptmenü" "Snapshot"

# [1/6] DDEV starten
echo -e "${YELLOW}[1/6] Starte DDEV...${NC}"
cd "$TOOLS_DIR/../woltlab-dev"
ddev start > /dev/null 2>&1
print_success "DDEV läuft${NC}\n"
...
if type safe_copy &>/dev/null; then
    safe_copy "$MAIN_DIR/woltlab-core/WCFSetup.tar.gz" "$PUBLIC_DIR/" 2>/dev/null || print_warning "WCFSetup.tar.gz nicht gefunden${NC}"
    safe_copy "$MAIN_DIR/woltlab-core/install.php" "$PUBLIC_DIR/" 2>/dev/null || print_warning "install.php nicht gefunden${NC}"
    safe_copy "$MAIN_DIR/woltlab-core/test.php" "$PUBLIC_DIR/" 2>/dev/null || print_warning "test.php nicht gefunden${NC}"
else
    cp "$MAIN_DIR/woltlab-core/WCFSetup.tar.gz" "$PUBLIC_DIR/" 2>/dev/null || print_warning "WCFSetup.tar.gz nicht gefunden${NC}"
    cp "$MAIN_DIR/woltlab-core/install.php" "$PUBLIC_DIR/" 2>/dev/null || print_warning "install.php nicht gefunden${NC}"
    cp "$MAIN_DIR/woltlab-core/test.php" "$PUBLIC_DIR/" 2>/dev/null || print_warning "test.php nicht gefunden${NC}"
fi
print_success "Public-Ordner geleert (nur WCFSetup.tar.gz + install.php)${NC}\n"
...
if ddev mysql -e "DROP DATABASE IF EXISTS $DB_NAME; CREATE DATABASE $DB_NAME CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>/dev/null; then
    print_success "Datenbank geleert und neu erstellt${NC}\n"
    ...
    print_success "Datenbank geleert und neu erstellt (mit Backticks)${NC}\n"
else
    print_warning "Datenbank-Operation fehlgeschlagen, versuche alternative Methode...${NC}"
    ...
    if [ -n "$MYSQL_PORT" ] && command -v mysql &> /dev/null; then
        ...
        print_success "Datenbank geleert und neu erstellt (via mysql Client)${NC}\n" || \
...
if type open_browser &>/dev/null; then
    if open_browser "$INSTALL_URL"; then
        print_success "Browser geöffnet${NC}\n"
    else
        print_warning "Browser konnte nicht automatisch geöffnet werden${NC}"
    ...
    if command -v firefox &> /dev/null; then
        ...
        print_warning "Kein Browser gefunden - bitte öffne manuell: $INSTALL_URL${NC}\n"
    fi
fi

print_section "BITTE INSTALLATION DURCHFÜHREN" "Hauptmenü" "Snapshot"
...
echo ""
print_section "Snapshot erfolgreich erstellt" "Hauptmenü" "Snapshot"
print_success "Snapshot erfolgreich erstellt!"
echo ""
